...
public class OpenAiRagSpringAiChatModelApplication {

	@Value("Artificial intelligence - Wikipedia.pdf")
	private Resource pdf;

	public static void main(String[] args) {
		SpringApplication.run(OpenAiRagSpringAiChatModelApplication.class, args);
	}

	@Bean
	VectorStore vectors(EmbeddingModel embeddingModel) {

		SimpleVectorStore vectors = SimpleVectorStore.builder(embeddingModel).build();
	...
	CommandLineRunner commandLineRunner(ChatModel chatModel, VectorStore vectors) {
		...
		return args -> {
			...
			String answer = chatModel.call(combinedPrompt).getResult().getOutput().getText();
